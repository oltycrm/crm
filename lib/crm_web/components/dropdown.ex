defmodule CrmWeb.Components.Dropdown do
  ...
  @doc """
    <.dropdown label="Dropdown" js_lib="alpine_js|live_view_js">
      <.dropdown_menu_item link_type="button">
        <Heroicons.Outline.home class="w-5 h-5 text-neutral-500" />
    ...
    >
      <div>
        ...
        >
          ...
          <%= if @label do %>
            <%= @label %>
            <Heroicons.Solid.chevron_down class="w-5 h-5 ml-2 -mr-1 dark:text-neutral-100" />
          <% end %>
      ...
      <div
        ...
        class={
          placement_class(@placement) <>
            " absolute z-30 w-56 mt-2 bg-white dark:bg-neutral-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
        }
        role="menu"
  ...
  defp trigger_button_classes(nil, nil),
    do:
      "flex items-center text-neutral-400 rounded-full hover:text-neutral-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-neutral-100 focus:ring-primary-500"

  defp trigger_button_classes(_label, nil),
    do:
      "inline-flex justify-center w-full px-4 py-2 text-sm font-medium text-neutral-700 bg-white border border-neutral-300 rounded-md shadow-sm dark:text-neutral-300 dark:bg-neutral-900 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800 hover:bg-neutral-50 focus:outline-none"

  defp trigger_button_classes(_label, _trigger_element), do: "align-middle"

  defp dropdown_menu_item_classes(),
    do:
      "block flex gap-2 items-center self-start dark:hover:bg-neutral-700 dark:text-neutral-300 justify-start px-4 py-2 text-sm text-neutral-700 transition duration-150 dark:bg-neutral-800 ease-in-out hover:bg-neutral-100 w-full text-left"
